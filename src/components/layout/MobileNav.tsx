...
import { Home, Search, ShoppingCart, User, Package, Store, Briefcase, Settings, LayoutDashboard, DollarSign, BarChart3 } from "lucide-react";
      ...
      <div className="flex items-center justify-around h-16 pb-safe">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path || 
            (item.path !== `/${role === "consumer" ? "customer" : role}` && location.pathname.startsWith(item.path));
